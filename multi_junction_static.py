...
def run():
    ...
    while step<1000:
        ...
        if (need_yellow1!=0)and(step==(last_traffic_update1+wait_duration1)):
            ...
            wait_duration1 = 1
            need_yellow1 = 0
        if (p1==0) and (step==(last_traffic_update1+wait_duration1)):
            traci.trafficlight.setPhase("n1",4)
            last_traffic_update1 = step
            wait_duration1 = 12
            need_yellow1 = 5
            p1=p1+1
        if (p1 == 1)and(step==(last_traffic_update1+wait_duration1)):
            traci.trafficlight.setPhase("n1",6)
            last_traffic_update1 = step
            wait_duration1 = 12
            need_yellow1 = 7	
            p1=p1+1
        if (p1 == 2)and(step==(last_traffic_update1+wait_duration1)):
            traci.trafficlight.setPhase("n1",0)
            last_traffic_update1 = step
            wait_duration1 = 12
            need_yellow1 = 1
            p1=p1+1
        if (p1 == 3)and(step==(last_traffic_update1+wait_duration1)):
        ...
        if (need_yellow2!=0)and(step==(last_traffic_update2+wait_duration2)):
            ...
            wait_duration2 = 1
            need_yellow2 = 0
        if (p2==0) and (step==(last_traffic_update2+wait_duration2)):
            traci.trafficlight.setPhase("n2",4)
            last_traffic_update2 = step
            wait_duration2 = 12
            need_yellow2 = 5
            p2=p2+1
        if (p2 == 1)and(step==(last_traffic_update2+wait_duration2)):
            traci.trafficlight.setPhase("n2",6)
            last_traffic_update2 = step
            wait_duration2 = 12
            need_yellow2 = 7	
            p2=p2+1
        if (p2 == 2)and(step==(last_traffic_update2+wait_duration2)):
            traci.trafficlight.setPhase("n2",0)
            last_traffic_update2 = step
            wait_duration2 = 12
            need_yellow2 = 1
            p2=p2+1
        if (p2 == 3)and(step==(last_traffic_update2+wait_duration2)):
        ...
        if (need_yellow3!=0)and(step==(last_traffic_update3+wait_duration3)):
            ...
            wait_duration3 = 1
            need_yellow3 = 0
        if (p3==0) and (step==(last_traffic_update3+wait_duration3)):
            traci.trafficlight.setPhase("n3",4)
            last_traffic_update3 = step
            wait_duration3 = 12
            need_yellow3 = 5
            p3=p3+1
        if (p3 == 1)and(step==(last_traffic_update3+wait_duration3)):
            traci.trafficlight.setPhase("n3",6)
            last_traffic_update3 = step
            wait_duration3 = 12
            need_yellow3 = 7	
            p3=p3+1
        if (p3 == 2)and(step==(last_traffic_update3+wait_duration3)):
            traci.trafficlight.setPhase("n3",0)
            last_traffic_update3 = step
            wait_duration3 = 12
            need_yellow3 = 1
            p3=p3+1
        if (p3 == 3)and(step==(last_traffic_update3+wait_duration3)):
        ...
        if (need_yellow4!=0)and(step==(last_traffic_update4+wait_duration4)):
            ...
            wait_duration4 = 1
            need_yellow4 = 0
        if (p4==0) and (step==(last_traffic_update4+wait_duration4)):
            traci.trafficlight.setPhase("n4",4)
            last_traffic_update4 = step
            wait_duration4 = 12
            need_yellow4 = 5
            p4=p4+1
        if (p4 == 1)and(step==(last_traffic_update4+wait_duration4)):
            traci.trafficlight.setPhase("n4",6)
            last_traffic_update4 = step
            wait_duration4 = 12
            need_yellow4 = 7	
            p4=p4+1
        if (p4 == 2)and(step==(last_traffic_update4+wait_duration4)):
            traci.trafficlight.setPhase("n4",0)
            last_traffic_update4 = step
            wait_duration4 = 12
            need_yellow4 = 1
            p4=p4+1
        if (p4 == 3)and(step==(last_traffic_update4+wait_duration4)):
...
if __name__=="__main__":
    ...
    plt.title("Queue Length vs Time Steps Static")
    #plt.legend()
    plt.show()
    ...
    plt.title("Jam Length vs Time Steps Static")
    #plt.legend()
    plt.show()
    ...
    plt.title("Mean Speed vs Time Steps static")
    #plt.legend()
    plt.show()
    # Averages
    x1 = []
